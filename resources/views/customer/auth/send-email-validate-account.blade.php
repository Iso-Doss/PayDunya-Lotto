{{-- resources/view/customer/auth/send-email-validate-account.blade.php --}}

@extends($profile . '.auth.base')

@section('title', __('Mail de validation de compte'))
...
@section('base.auth.form-title', __('Mail de validation de compte'))
@section('base.auth.form-sub-title', __('Pour recevoir un nouveau mail pour valider votre compte, saisissez votre adresse électronique ci-dessous.'))
...
@section('base.auth.form')
    ...
    <form action="{{ route($profile . '.auth.send-email-validate-account') }}" method="post" class="" novalidate>
        ...
        <div class="align-items-center mt-0">
            <div class="d-grid">
                <button class="btn btn-primary mb-0" type="submit">
                    {{ __('Demande de validation de compte') }}
                </button>
            </div>
        </div>
    </form>
