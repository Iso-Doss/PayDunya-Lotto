{{-- resources/view/administrator/dashboard/transaction/index.blade.php --}}

@extends($profile . '.dashboard.base')

@section('sub-title', __('Transactions'))
    ...
    <div class="page-content-wrapper border">
        ...
        <div class="card-body">
            ...
            <div class="row g-3 align-items-center justify-content-between mt-1 mb-4">
                <!-- Content -->
                <div class="col-md-12">
                    <form class="rounded position-relative" action="{{ route($profile . '.transaction.index') }}">
                        @csrf
                        ...
                        <div class="row g-3">
                            ...
                            <!-- User -->
                            <div class="col-xl-6">
                                <label for="transaction-user-id" class="form-label">
                                    {{ __('Utilisateur : ') }}
                                </label>
                                @if(!empty($users))
                                    <select name="user_id" id="transaction-user-id"
                                            class="form-select select2bs4 me-1 mt-2 transaction-user-id @error('user_id'){{'is-invalid'}}@enderror">
                                        <option value="">
                                            {{ __('Veuillez sélectionner l\'utilisateur') }}
                                        </option>
                                        @foreach($users as $user)
                                            <option
                                                value="{{ $user->id }}" @selected(($input['user_id'] ?? '') == $user->id)>
                                ...
                                @else
                                    <p class="me-1 mt-3">
                                        {{ __('Aucun utilisateur n\'est disponible pour le moment.') }}
                                    </p>
                                @endif
                            </div>
