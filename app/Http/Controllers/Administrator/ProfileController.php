...
class ProfileController extends Controller
{
    ...
    public function updateEmail(UpdateEmailProfileRequest $request): RedirectResponse
    {
        ...
        try {
            PasswordResetTokens::create(['email' => Auth::user()->email, 'new_email' => $request->input('email'), 'profile' => $request->input('profile') ?? 'ADMINISTRATOR', 'token' => $token, 'type' => 'UPDATE-EMAIL']);
        } catch (QueryException) {
            $passwordResetToken = PasswordResetTokens::where('email', Auth::user()->email)->where('new_email', $request->input('email'))->where('profile', $request->input('profile') ?? 'ADMINISTRATOR')->where('type', 'UPDATE-EMAIL')->first();
    ...
    public function validateUpdateEmail(string $email, string $new_email, string $token): View|RedirectResponse
    {
        ...
        $user = User::whereEmail($email)->whereProfile('ADMINISTRATOR')->first();

        $validateAccountToken = PasswordResetTokens::where('email', $email)->where('profile', 'ADMINISTRATOR')->where('token', $token);
